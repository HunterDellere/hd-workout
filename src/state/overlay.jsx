...
export function OverlayProvider({ children }) {
  ...
  useEffect(() => {
    ...
    loadFromStorage(STORAGE_KEYS.programOverlay, {}).then((value) => {
      if (cancelled) return;
      setOverlay(value && typeof value === 'object' ? value : {});
      setHydrated(true);
    });
  ...
  useEffect(() => {
    if (!hydrated) return;
    saveToStorage(STORAGE_KEYS.programOverlay, overlay);
  }, [overlay, hydrated]);

  // Effective hydrated days for the active program — re-derived only when
  // overlay changes. Cheap (a few hundred allocations) but worth memoizing.
  const days = useMemo(() => {
